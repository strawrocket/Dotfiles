...
export interface Props {
	...
	onModeChange?: {
		(mode: CardMode): void;
		(mode: "paraphrase", textHighlightType?: ToolboxTextSelectionType, triggerType?: CardTriggerType): void;
	};
	onClose: () => void;
...
const SettingsSelector: React.FC<SettingsSelectorProps> = ({ onDisableLollipopSelected }) => {
	...
	const { current: i18n } = useRef<Record<"close" | "sendFeedback" | "disableLollipop", string>>({
	...
	const toggleMenu = useStopPropagation(() => setMenuVisible(!menuVisible));
...
const CardBase = forwardRef<CardBaseRef, React.PropsWithChildren<Props>>(function CardBase(
	...
	const handleDisableLollipopSettingsOptionClick = useStopPropagation(() => {
		...
		TrackerAdapter.trackEvent("Action", "disable_lollipop");

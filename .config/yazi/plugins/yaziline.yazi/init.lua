local function setup(_, options)
	...
	function Status:size()
		...
		return ui.Span(current_separator_style.separator_close .. " " .. ya.readable_size(h:size() or h.cha.len) .. " ")
			...
			:bg(THEME.which.separator_style.fg)
	end
	...
	function Status:name()
		...
		local trimmed_name = #h.name > config.filename_max_length
				and (string.sub(h.name, 1, config.filename_trim_length) .. "..." .. string.sub(
					h.name,
					-config.filename_trim_length
				))
			or h.name
	...
	function Status:files()
		...
		local yanked_fg = files_yanked > 0
				and (files_is_cut and THEME.manager.count_cut.bg or THEME.manager.count_copied.bg)
			or THEME.which.separator_style.fg

		local yanked_text = files_yanked > 0 and config.yank_symbol .. " " .. files_yanked or config.yank_symbol .. " 0"

		return ui.Line({
			ui.Span(" " .. current_separator_style.separator_close_thin .. " "):fg(THEME.which.separator_style.fg),
	...
	function Status:modified()
		...
		return ui.Span(os.date("%Y-%m-%d %H:%M", time) .. " " .. current_separator_style.separator_open_thin .. " ")
			:fg(THEME.which.separator_style.fg)
	end
